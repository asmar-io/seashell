...
const MintNFTForm = () => {
  ...
  const packagid =
    "0x576ce6f9227b55f93844988881ecb53c74c8ffcbd5e7ecf6be8624d2ebd47f25";
  const allowlist_pckg_id =
    "0x87e99606517763f4ba82d618e89de5bd88063e49d0c75358bf2af392782f99fd";
  const id =
    "0x97fad43945130f277532b7891d47a81823d7990af6795b0ec4f9364c474eefda";
  ...
  async function uploadtoblob(
    ...
    const encryptedCoords = await encryption(
      coordsString,
      allowlistObjectId ?? ""
    ); // Use allowlist object ID, not cap ID
    ...
    const fileBuffer = new Blob([JSON.stringify(payloadWithMetadata)], {
  ...
  async function uploadtomarketplace(blob: string) {
    ...
    tx.moveCall({
      arguments: [
        tx.pure.vector("u8", toUint8array(`${name}`)),
        tx.pure.vector("u8", toUint8array(`${description}`)),
        tx.pure.vector("u8", toUint8array(`${uploadedImageUrl}`)),
        tx.pure.vector("u8", toUint8array(`${merkleRoot}`)),
      ],
  ...
  async function trialfetch() {
    ...
    const filteredOwnedObjects = ownedobjects.data.filter((obj) => {
      ...
      if (
        obj.data?.type ==
        "0x576ce6f9227b55f93844988881ecb53c74c8ffcbd5e7ecf6be8624d2ebd47f25::allowlist::Cap"
      ) {
        return true;
      }
      return false;
  ...
  async function createAllowlist(nftName: string) {
    ...
    try {
      ...
      // @ts-ignore
      const objects_allowlist_ids = objects.map(
        (obj) => obj.data?.content?.fields?.allowlist_id
      );
      console.log("Objects Allowlist IDs:", objects);
      const final_allowlist_ids = await Promise.all(
        objects_allowlist_ids.map(async (id) => {
      ...
      const allowlistid =
        objects2.filter(
          (obj) => obj.data?.content?.fields?.allowlist_id === allowlistObjectId
        )[0]?.data?.objectId || "";
      console.log("Allowlist Object ID:", allowlistObjectId);
